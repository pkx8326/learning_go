...
func writeBalanceToFile(updatedBalance float64) {
	balanceText := fmt.Sprint(updatedBalance)
	os.WriteFile(accountBalanceFile, []byte(balanceText), 0644) // 0644 is the file permission (read-write allowed for owner only)

}

func readBalanceFromFile() (updatedBalance float64) {
	balanceData, err := os.ReadFile(accountBalanceFile)
	if err != nil {
		fmt.Println("Balance data not found.")
	}
	balanceText := string(balanceData)
	updatedBalance, err = strconv.ParseFloat(balanceText, 64)
	if err != nil {
		fmt.Println("Data type mistmatched.")
	}
	return
}
